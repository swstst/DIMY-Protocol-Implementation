...
class Client:
    ...
    def receiver(self):
        print(
            "-" * 15, "RECV: Started listening on port", self.UDP_RECV_PORT, end="\n\n"
        )
        ...
        while not self.stop_event.is_set():
            ...
            for hash_id, ephID_shares in shares.items():
                ...
                print(
                    ...
                    self.CLIENT_ID,
                    "-" * 15,
                    "!! Successful reconstruction of EphID",
                    valid_ephID,
                    end="\n\n",
                )
    ...
    def _upload_cbf(self):
        CBF, oldest_date = self._combine_DBFs()
    ...
    def run(self):
        ...
        gen_EphID_shares_thread = threading.Thread(
            target=self.gen_EphID_shares_every_t, daemon=True
        )
